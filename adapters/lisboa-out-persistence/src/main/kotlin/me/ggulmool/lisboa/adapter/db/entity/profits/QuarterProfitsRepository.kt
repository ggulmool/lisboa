...
interface QuarterProfitsRepository: JpaRepository<QuarterProfitsEntity, Long> {
    ...
    @Query(
        """
            select new me.ggulmool.lisboa.adapter.db.entity.profits.QuarterProfitsDto(stock.stockNo, quarter_profits.year, quarter_profits.quarter, quarter_profits.profits) from QuarterProfitsEntity quarter_profits 
            join quarter_profits.stockEntity stock
            where stock.stockNo = :stockNo
        """
    )
